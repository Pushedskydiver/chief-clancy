...
**Session 79 (Step 0 execution):** 64-tuple corpus reconstructed (PRs #291/#278/#283) + hardened DA run against 35 in-gate + 5 FP samples. End-to-end recall **65.7% (23/35)**, gate PASS (+15.7pp above 50% threshold). FP rate 0/5 real (1 novel catch). Fix-overcorrection subset weaker at 43%. **Decision: ship DA-hardening only** — PR #293 (Claim-extraction pass) is the entire Option-A deliverable. All Step 1+ work (subagent, LSP, mechanism spike, agent-hook) cancelled. Full protocol + results at `.claude/research/option-a/eval/step-0-results.md`.

**Next:** Wiring-claim direction audit discipline (addresses 28pp recall gap on under-claims) — this PR. After that, pick between [`project_readability_review`](~/.claude/.../memory/project_readability_review.md) (code-readability pass) and [`project_docs_local_pipeline_update`](~/.claude/.../memory/project_docs_local_pipeline_update.md) (refresh all docs for the shipped local pipeline).
