...
## Next workstreams (after Session 128)

Ordering updated 2026-04-24 (Session 128 — 4 PRs shipped: [PR #395](https://github.com/Pushedskydiver/chief-clancy/pull/395) 4-way scan-declarer taxonomy across 5 surfaces (coordinated-edit scope call, closed both Session-127 adjacent observations); [PR #396](https://github.com/Pushedskydiver/chief-clancy/pull/396) codified 3 review-gate protocols — sibling-doc kept-prose invalidation (n=4 PROMOTION-READY), DA-reports-in-chat (novel n=1), surrogate dispatch timing + audit-trail (novel n=1); [PR #397](https://github.com/Pushedskydiver/chief-clancy/pull/397) `import.meta.resolve()` precision fix on `docs/ARCHITECTURE.md:27`; [PR #398](https://github.com/Pushedskydiver/chief-clancy/pull/398) user-reported duplicate `CLANCY_LABEL_PLAN` prompt bug — removed redundant GitHub branch from init + settings workflows, `@chief-clancy/terminal` patch changeset added (user corrected an initial `skip-changeset` misapplication). Dependabot β window still inactive at session close — 0 Dependabot PRs across Sessions 125-128 (5 days elapsed; Monday UTC tick 2026-04-27 pending).
...
2. **Documentation drift follow-ups — ITEM 1 FULLY COMPLETE (Sessions 112-124); follow-up carry-forward sweep EXTENDED AND FULLY CLOSED Sessions 126-128.** 13 item-1 PRs shipped Sessions 112-124. Session 126 closed 3 named out-of-scope carry-forwards via PR #388 + #389. Session 127 closed the remaining 3 named carry-forwards plus 2 novel ones surfaced by reviews (PR #390-#394). Session 128 closed the 2 adjacent observations logged by Session 127 via [PR #395](https://github.com/Pushedskydiver/chief-clancy/pull/395) (4-way scan-declarer taxonomy across 5 surfaces, Alex-merged due to `/packages/*/package.json` blast-radius) and [PR #397](https://github.com/Pushedskydiver/chief-clancy/pull/397) (`ARCHITECTURE.md:27` `import.meta.resolve()` → `at installer-time` precision drop; auto-merged). Also this session: [PR #398](https://github.com/Pushedskydiver/chief-clancy/pull/398) user-reported duplicate `CLANCY_LABEL_PLAN` prompt bug on `npx chief-clancy` init + `/clancy:settings` menu; 2 workflow files (`init.md` + `settings.md`), Pass-1 fold (Step 4c → Step 4c-2 precision + removed incorrect `skip-changeset` label + added `@chief-clancy/terminal` patch changeset), auto-merged.
   - **Named carry-forwards + Session-127 adjacent observations resolved.** Plus the PR #398 drift-fix closed a long-latent user-facing bug.
   - **NEW drift-fix backlog — `needs-refinement` / `CLANCY_PLAN_LABEL` cluster (surfaced by PR #398 sibling sweep):** 4 surfaces still reference the deprecated `CLANCY_PLAN_LABEL` env var and/or its `needs-refinement` default as if current. Not caused by PR #398's edit (pre-existing per sibling-kept-prose rule's "caused by this edit" scoping; legitimately deferred). Good candidate for one bundled follow-up drift-fix PR:
     - `packages/plan/src/workflows/plan.md:627` — user-facing empty-queue guidance still tells GitHub users to apply `needs-refinement` (load-bearing: Claude shows this prose on empty queue).
     - `packages/terminal/src/roles/setup/workflows/scaffold.md:517-518` — `.env.example` comment frames deprecated `CLANCY_PLAN_LABEL` as current without deprecation marker (asymmetric with scaffold.md L431/L556/L842 which correctly mark as deprecated).
     - `docs/roles/PLANNER.md:141` — config table row shows `needs-refinement` + deprecated `CLANCY_PLAN_LABEL` env var.
     - `docs/guides/CONFIGURATION.md:199` — "Label: `clancy:plan` (or `needs-refinement`)" — defensible as documenting fallback, arguably tighten.
   ...
   - Low-priority date-stamp sweeps on stale "Last reviewed" headers (CONVENTIONS, SELF-REVIEW, DA-REVIEW, RATIONALIZATIONS, REVIEW-PATTERNS) can batch as a single nit-sweep.
3. **Review-gate protocol codification — NEW THIS SESSION (PR #396, 2026-04-24 `945e4f4`).** Three observed patterns promoted from transient surfaces into permanent policy-doc prose: (a) sibling-doc kept-prose invalidation as new 4th sub-class under `docs/DA-REVIEW.md §Cross-doc consistency sweep` (opener split into Reactive `a-c` + Proactive `d` modes); (b) DA-reports-in-chat-only discipline as new `§Reporting channel` subsection in DA-REVIEW.md + new `Key disciplines` bullet in `.claude/agents/da-review.md`; (c) surrogate dispatch timing (pre-merge slot) + audit-trail discipline as new paragraph in `docs/DEVELOPMENT.md §Post-PR flow`. Companion sync: `docs/SELF-REVIEW.md:115` companion-clause enumeration widened 3→4 sub-classes (self-caught kept-prose-invalidation applied to the rule it was codifying — R2 spec-grill surfaced). 4-pass soft-reset-to-fold sequence (R1 + R2 spec-grill + pass-3 post-DA+surrogate + pass-4 post-final-verification) — first 4-pass PR on record. Test in Session 129+: these rules are now authoritative; watch for drift or rule-self-apply catches on subsequent PRs.
