...
Check for updates to `@chief-clancy/plan`, display what's new, and install the latest version. Does **not** touch hooks, `settings.json`, `CLAUDE.md`, `.gitignore`, `.prettierignore`, or the `.clancy/` folder.

---

## Step 1 — Read installed version

Check both locations for the `VERSION.plan` marker file:
...
```
Clancy Plan is not installed. Run `npx @chief-clancy/plan` first.
```
...
```bash
npm view @chief-clancy/plan version 2>/dev/null
```
...
To update manually: npx -y @chief-clancy/plan@latest [--local/--global]
```
...
```bash
# Tag: @chief-clancy/plan@0.3.5 → URL-encoded: %40chief-clancy%2Fplan%400.3.5
curl -sf "https://api.github.com/repos/Pushedskydiver/chief-clancy/releases/tags/%40chief-clancy%2Fplan%40{latest}"
```
...
⚠️  Terminal pipeline detected (VERSION marker found).
    ...
    Proceeding will update plan-specific files and overwrite shared scan files.
...
If other standalone packages are installed (brief or dev) but terminal is NOT installed:
...
Determine the install scope based on where `VERSION.plan` was found:
...
This will run the plan installer which overwrites:
...
| Mode     | Command(s)                                     |
| -------- | ---------------------------------------------- |
| `local`  | `npx -y @chief-clancy/plan@latest --local`     |
| `global` | `npx -y @chief-clancy/plan@latest --global`    |
...
Re-read the `VERSION.plan` marker from the same location(s) as Step 1.
...
```
✅ @chief-clancy/plan updated: v{old} → v{new}

"Plan upgraded, Chief." — Start a new Claude Code session to pick up the updated commands.
```
