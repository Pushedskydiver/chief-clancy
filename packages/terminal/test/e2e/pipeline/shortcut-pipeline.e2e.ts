 ...
 *
 * Creates a real Shortcut story, runs the full pipeline with Claude
 ...
 * checks GITHUB_TOKEN + GITHUB_REPO before Shortcut and would
 * misdetect as GitHub Issues. GITHUB_TOKEN alone is used as the
 * git host token for PR creation (via sharedEnvSchema).
 *
 * NOTE: The env var name difference — .env.e2e uses SHORTCUT_TOKEN
 * but the runtime schema expects SHORTCUT_API_TOKEN. The credential
 * loader reads the former; setupE2EPipeline writes the latter.
 */
...
import { cleanupShortcutTicket } from '../helpers/cleanup/shortcut.js';
import {
...
describe.skipIf(!canRun)('E2E: Shortcut — full pipeline', () => {
  ...
  beforeAll(async () => {
    ...
    // Create a real test story on the Shortcut workspace
    const ticket = await createTestTicket('shortcut', runId);
    ticketId = ticket.id;
  ...
  afterAll(async () => {
    ...
    if (ticketId) {
      // Shortcut cleanup takes the numeric story ID
      await cleanupShortcutTicket(ticketId).catch(() => {});
    }
    pipeline?.repo.cleanup();
    cleanupGitAuth();
  });

  it('creates a story, runs the pipeline, and verifies PR creation', async () => {
    const githubCreds = getGitHubCredentials()!;
    const shortcutCreds = getShortcutCredentials()!;

    // Set up pipeline with real GitHub remote + Shortcut credentials.
    // GITHUB_REPO is intentionally omitted — see file-level JSDoc.
    // SHORTCUT_API_TOKEN is the runtime env var name (not SHORTCUT_TOKEN).
    ...
    // Run the full pipeline — real Shortcut + GitHub API, simulated Claude
