...
const buildSystemPrompt = (designContext: string): string =>
  [
    ...
    '</variant>',
    '',
    'Slot-anchor contract:',
    'Every interactive element and major structural element in the variant',
    'MUST include a stable `data-clancy-slot="<unique>"` attribute (analogous',
    "to shadcn/ui's `data-slot` convention). When iterating on a prior variant,",
    'PRESERVE existing data-clancy-slot values on retained elements.',
    'Generate fresh slot IDs only for newly-introduced elements.',
    '',
    'Variant CSS strategy:',
    'Each variant renders standalone inside a sandboxed iframe with no shared',
    'build pipeline. Use a scoped <style> block at the top of the variant',
    'HTML with CSS variables for theming.',
    'Do NOT use Tailwind utility classes.',
    'Do NOT link external stylesheets.',
    'Do NOT reference any CDN.',
    'The variant must be self-contained HTML+CSS.',
    '',
    '---',
    '',
    designContext,
