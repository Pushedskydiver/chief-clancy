...
<rationale>Attribute order should not matter.</rationale>
  ...
  it('system prompt mandates data-clancy-slot on every interactive and structural element + preservation across iterations (slice 19 spec L437-441 tightening)', async () => {
    const { client, createMock } = buildClient(VALID_RESPONSE);
