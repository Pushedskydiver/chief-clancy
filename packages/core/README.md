...
> [!WARNING]
> **Not supported for standalone use.** This package is consumed transitively by the `@chief-clancy/*` wrappers via `workspace:*`. Its public API may change without notice.
>
> **Deep-import surface (as of 0.2.0):** consumers may import from the package root (`@chief-clancy/core`) or the four namespaced subpaths: `@chief-clancy/core/types/*.js`, `@chief-clancy/core/schemas/*.js`, `@chief-clancy/core/shared/*.js`, `@chief-clancy/core/board/*.js`. The `*` matches nested paths — e.g. `@chief-clancy/core/board/github/api/index.js` resolves. Top-level deep imports (`@chief-clancy/core/foo.js`) are no longer resolvable — the 0.1.x `./*.js` wildcard export has been replaced by the four subdirectory wildcards above.
