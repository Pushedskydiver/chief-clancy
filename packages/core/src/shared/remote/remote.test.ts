...
describe('parseRemote', () => {
  ...
  it('parses GitHub HTTPS URL', () => {
    expect(parseRemote('https://github.com/Pushedskydiver/clancy.git')).toEqual(
      {
        host: 'github',
  ...
  it('parses Bitbucket Server /scm/ URL', () => {
    expect(parseRemote('https://bitbucket.acme.com/scm/PROJ/repo.git')).toEqual(
      {
  ...
  it('returns unknown for empty string', () => {
    expect(parseRemote('')).toEqual({ host: 'unknown', url: '' });
  });
  ...
  it('always returns a valid host field', () => {
    ...
    fc.assert(
      fc.property(fc.string({ minLength: 0, maxLength: 200 }), (raw) => {
  ...
  it('round-trips GitHub HTTPS URLs', () => {
    fc.assert(
      fc.property(
...
describe('buildApiBaseUrl', () => {
  ...
  it('returns undefined for none host', () => {
    expect(buildApiBaseUrl({ host: 'none' })).toBeUndefined();
  });
