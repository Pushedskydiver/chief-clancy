...
describe('rework-comment property-based', () => {
  it('isReworkComment returns true for any string prefixed with "Rework:"', () => {
    fc.assert(
      fc.property(fc.string({ minLength: 0, maxLength: 200 }), (content) => {
        const comment = `Rework: ${content}`;
        return isReworkComment(comment) === true;
      }),
    );
  });

  it('extractReworkContent round-trips with isReworkComment', () => {
    fc.assert(
      fc.property(
        fc
        ...
        (content) => {
          const comment = `Rework: ${content}`;
          const extracted = extractReworkContent(comment);
          return isReworkComment(comment) && extracted === content.trim();
        },
      ),
    );
  });

  it('isReworkComment returns false for strings not starting with rework:', () => {
    fc.assert(
      fc.property(
        fc
          .string({ minLength: 0, maxLength: 200 })
          .filter((s) => !/^\s*rework:/i.test(s)),
        (comment) => isReworkComment(comment) === false,
      ),
    );
  });
});
