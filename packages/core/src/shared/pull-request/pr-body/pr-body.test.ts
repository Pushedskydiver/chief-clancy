...
describe('buildPrBody', () => {
  it('includes Jira link for Jira boards', () => {
    ...
    const body = buildPrBody({ config, ticket: baseTicket });
  ...
  it('includes Closes #N for GitHub boards targeting base branch', () => {
    ...
    const body = buildPrBody({ config, ticket, targetBranch: 'main' });
  ...
  it('uses Part of for GitHub boards targeting epic branch', () => {
    ...
    const body = buildPrBody({ config, ticket, targetBranch: 'epic/proj-100' });
  ...
  it('uses Part of for GitHub boards targeting milestone branch', () => {
    ...
    const body = buildPrBody({
      config,
      ticket,
      targetBranch: 'milestone/sprint-3',
    });
    expect(body).toContain('Part of #42');
  });
  ...
  it('uses Ticket label for Shortcut boards', () => {
    const config: BoardConfig = {
      provider: 'shortcut',
      env: { SHORTCUT_API_TOKEN: 'tok' },
    };
    const ticket: Ticket = {
      ...baseTicket,
      key: 'sc-123',
    ...
    const body = buildPrBody({ config, ticket });
    expect(body).toContain('**Ticket:** sc-123');
  });

  it('uses Ticket label for Notion boards', () => {
    const config: BoardConfig = {
      provider: 'notion',
      env: { NOTION_TOKEN: 'tok', NOTION_DATABASE_ID: 'db-id' },
    };
    const ticket: Ticket = {
      ...baseTicket,
      key: 'NOT-1',
    ...
    const body = buildPrBody({ config, ticket });
    expect(body).toContain('**Ticket:** NOT-1');
  });

  it('uses Ticket label for Azure DevOps boards', () => {
    const config: BoardConfig = {
      provider: 'azdo',
      env: { AZDO_ORG: 'org', AZDO_PROJECT: 'proj', AZDO_PAT: 'pat' },
    };
    const ticket: Ticket = {
      ...baseTicket,
      key: 'AB#42',
    ...
    const body = buildPrBody({ config, ticket });
    expect(body).toContain('**Ticket:** AB#42');
  });

  // ─── singleChildParent ────────────────────────────────────────────────────

  describe('singleChildParent option', () => {
  ...
  describe('epicContext option', () => {
    ...
    it('includes epic banner when targeting epic branch with context', () => {
      ...
      const body = buildPrBody({
        config: githubConfig,
        ticket: githubTicket,
        targetBranch: 'milestone/49',
        epicContext: epic,
      });
      expect(body).toContain('## Part of epic #49');
      expect(body).toContain(
        '3 siblings previously delivered to `milestone/49`',
      );
...
describe('pr-body property-based', () => {
  const githubConfig: BoardConfig = {
    provider: 'github',
    env: { GITHUB_TOKEN: 'ghp_test', GITHUB_REPO: 'owner/repo' },
  };

  it('buildPrBody always contains the Clancy footer', () => {
    fc.assert(
      fc.property(
        ...
        (key, description) => {
          ...
          const body = buildPrBody({ config: githubConfig, ticket });
          return body.includes('Clancy') && body.includes('</details>');
        },
      ),
    );
  });

  it('buildPrBody always contains the ticket key', () => {
    fc.assert(
  ...
  it('isEpicBranch returns true iff branch starts with epic/ or milestone/', () => {
    fc.assert(
      fc.property(fc.string({ minLength: 0, maxLength: 100 }), (suffix) => {
        const epicResult = isEpicBranch(`epic/${suffix}`);
        const milestoneResult = isEpicBranch(`milestone/${suffix}`);
        return epicResult === true && milestoneResult === true;
      }),
    );
  });
});
