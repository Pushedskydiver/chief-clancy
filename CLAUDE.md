...
- **At session start: check the PROGRESS.md detail band before proposing a primary workstream.** If the band (between `## Next workstreams` and `## Session archive`) contains more than 5 full entries OR exceeds ~10k tokens, archive overflow to `docs/history/SESSIONS.md` first. Multi-session catch-up is one §12 trigger, not several. Handoff-time check is the backstop, not the primary firing. **Procedure:** `docs/DEVELOPMENT.md` "Archival maintenance".
