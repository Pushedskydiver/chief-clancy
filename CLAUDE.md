...
- **Consult INDEX before policy-adjacent edits.** Before Edit/Write on `CLAUDE.md`, `docs/**/*.md`, `.claude/agents/*.md`, or `.github/copilot-instructions.md`: identify the matching `docs/INDEX.md` scenario, then enumerate affected siblings + protocol steps before proceeding. **Whenever you edit `CLAUDE.md`, also apply the same change to `AGENTS.md` modulo the adaptation set in `docs/DEVELOPMENT.md §AGENTS.md ↔ CLAUDE.md sync` — the two files must stay byte-equal modulo the table.** After `gh pr merge` of a PR containing `.changeset/*.md`: consult `docs/INDEX.md` §10 before closing the task.
