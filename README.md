...
> [!WARNING]
> Clancy is in early development. Expect bugs, breaking changes, and rough edges. If you hit an issue, please [open a bug report](https://github.com/Pushedskydiver/chief-clancy/issues/new).
...
[What it does](#what-it-does) · [Install](#install) · [Commands](#commands) · [Supported boards](#supported-boards) · [Comparison](./docs/COMPARISON.md) · [Contributing](./CONTRIBUTING.md)

---

![Clancy terminal preview](assets/terminal-preview.svg)

Named after Chief Clancy Wiggum (The Simpsons) — built on the [Ralph technique](https://ghuntley.com/ralph/) by Geoffrey Huntley. [See credits →](./CREDITS.md)
...
| `/clancy:approve-plan` ¹               | Board: promote the approved plan to the ticket description. Local: write a sibling `.approved` marker (SHA-256 + timestamp) — a write-side contract today; runtime gate enforcement is deferred                                                                  |
...
| `/clancy:implement --from <plan.md>`   | Implement a single local plan — no board needed. Approval is enforced manually or by convention today (runtime `.approved` marker gate is deferred)                                                                                                              |
| `/clancy:implement --from <dir> --afk` | Local batch (wired via `runImplementBatch`): naturally sorts `.md` plans in the directory, skips any without a sibling `.approved` marker, executes approved plans sequentially. Marker SHA-256 verification is not yet wired — only marker existence is checked |
...
| **Planner** _(optional)_    | Refine backlog tickets into structured implementation plans     | `/clancy:plan`, `/clancy:approve-plan`                      |
| **Strategist** _(optional)_ | Grill → brief → vertical slices → board tickets                 | `/clancy:brief`, `/clancy:approve-brief`                    |
...
- [Implementer](docs/roles/IMPLEMENTER.md) — picks up tickets, implements, commits, creates PRs (AFK loop support)
...
| -------------------------------------------------- | -------------------------------------------------------------------------- |
| [Architecture](docs/ARCHITECTURE.md)               | Package hierarchy, directory structure, pipeline phases, board abstraction |
| [Visual Architecture](docs/VISUAL-ARCHITECTURE.md) | Mermaid diagrams — flows, roles, hooks, delivery paths                     |
| [Technical Reference](docs/TECHNICAL-REFERENCE.md) | Implementation details, board gotchas, env var behaviour                   |
| [Testing](docs/TESTING.md)                         | Vitest configs, test patterns, E2E setup, CI schedule                      |
| [Lifecycle](docs/LIFECYCLE.md)                     | End-to-end ticket journey from idea to merged code                         |
| [Configuration](docs/guides/CONFIGURATION.md)      | All env vars, queue filters, pipeline labels                               |
| [Security](docs/guides/SECURITY.md)                | Permissions, credential guard, token scopes                                |
| [Troubleshooting](docs/guides/TROUBLESHOOTING.md)  | Common issues and solutions                                                |
| [Comparison](docs/COMPARISON.md)                   | Clancy vs GSD vs PAUL                                                      |
| [Credits](CREDITS.md)                              | The Ralph technique and Geoffrey Huntley                                   |
