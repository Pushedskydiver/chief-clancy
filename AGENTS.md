...
Autonomous, board-driven development for Codex. Monorepo for `@chief-clancy/*` packages.
...
- **Consult INDEX before policy-adjacent edits.** Before Edit/Write on `AGENTS.md`, `docs/**/*.md`, `.codex/agents/*.toml`, or `.github/copilot-instructions.md`: identify the matching `docs/INDEX.md` scenario, then enumerate affected siblings + protocol steps before proceeding. **Whenever you edit `AGENTS.md`, also apply the same change to `CLAUDE.md` modulo the adaptation set in `docs/DEVELOPMENT.md §AGENTS.md ↔ CLAUDE.md sync` — the two files must stay byte-equal modulo the table.** After `gh pr merge` of a PR containing `.changeset/*.md`: consult `docs/INDEX.md` §10 before closing the task.
...
- **At session start: check the PROGRESS.md detail band before proposing a primary workstream.** If the band (between `## Next workstreams` and `## Session archive`) contains more than 5 entries OR exceeds ~10k tokens, archive overflow to `docs/history/SESSIONS.md` first. Multi-session catch-up is one §12 trigger, not several. Preemptive-at-handoff is the secondary mode and cannot substitute for the at-load check. **Procedure:** `docs/DEVELOPMENT.md` "Archival maintenance".
