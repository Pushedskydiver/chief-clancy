...
- Do cross-document citations use section names or heading anchors (e.g. `DEVELOPMENT.md §Two-phase grill discipline`) rather than line numbers (e.g. `DEVELOPMENT.md:278`)? Line numbers go stale as soon as the target file is edited — section names survive churn. Line-number refs are fine for same-file or same-diff references where the author controls both sides. _Caught by Copilot: PR #319 (a stale `:64` line-number ref to PROGRESS.md went stale within one session), PR #321 (a `:278` line-number ref to DEVELOPMENT.md in a prompt drifted on the next edit)._
...
- Do new modules respect the dependency direction? (core ← dev ← terminal ← chief-clancy; brief, plan, scan, and design are standalone)
...
- **§Flatten-boundary intra-wrapper grep.** Before `rmdir W/`, grep within `W/` for cross-sibling + parent-escaping references. Six regex patterns to run with `rg` (ripgrep) or `grep -E` / `egrep` — these use ERE operators (`+`, `|`, `(...)`) that plain BRE `grep` treats as literals, so the tool matters; patterns are NOT literal import strings (the `\.` escapes the dot for regex): `from '\.\./[^'\"]+'` (relative parent escape), `from '~/[a-z]/[^'\"]+'` (alias parent escape), `vi\.mock\(['\"]\.\./[^'\"]+['\"]` (relative mock string), `vi\.mock\(['\"]~/[a-z]/[^'\"]+['\"]` (alias mock string), `await import\(['\"](\.\./|~/[a-z]/)[^'\"]+['\"]` (dynamic imports), `export[^;]+from ['\"](\.\./|~/[a-z]/)[^'\"]+['\"]` (re-export forms). For each hit, resolve the sibling's **commit-fate** — which commit in the sequence flattens-or-deletes the sibling: co-flattening this commit / flattening later / barrel-deleted later — and **compound-transform** the path (apply both the sibling's path change AND the current wrapper's path change in one rewrite). Apply atomically in the same commit as the flatten, before the `rmdir` — typecheck gate between commits rejects dangling references. Evidence: surfaced in [Barrier-Core](../PROGRESS.md) plan grill (R2 caught `../sibling/` + `vi.mock()` gaps; R3 caught alias-axis symmetry; both within discovery iterations); shipped clean through Barrier-Core execution with zero §Flatten-boundary-class consumer-path errors. **n=1 applied** (Barrier pilot's simpler scope didn't surface the defect class); promoted alongside the n=2-applied rules below on rule-emergence + clean-execution evidence. A second application run is still needed to hit the strict "applied in 2 runs" bar; the rule above documents the current best-guess mechanics pending that second application.
- **Madge cycle-baseline diff, not absolute zero.** Before a mechanical refactor starts, record `madge --circular <packages>` against `main` as the cycle baseline. The acceptance gate is zero NEW cycles vs baseline, not zero cycles absolute. Pre-existing cycles in unrelated packages don't block the refactor unless scope includes remediation. Evidence: Barrier pilot post-commit-2 surfaced 3 cycles that matched `main` exactly (would have false-positived an absolute-zero gate); Barrier-Core verification reproduced the pattern (same 3, unchanged). n=2 applied.
- **Name-collision audit before bulk sed.** When mechanical sed patterns risk matching name-collision targets (`X/X.ts` files elsewhere in the tree), anchor patterns to full paths from the package root OR use alternation that excludes the collision set. Audit `git status` after each bulk pass — any mutated file whose path isn't in the expected set is a stop-the-line. Evidence: Barrier pilot hit 2 collision corruptions during execution (caught + fixed pre-merge via diff audit); Barrier-Core pre-emptively excluded the `azdo` multi-file folder from the schemas-flatten sed alternation — zero collision defects. n=2 applied.
