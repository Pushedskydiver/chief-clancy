...
- **Platform parity check** — when listing supported platforms (board providers, git hosts, etc.), check `feedback_azure_parity.md` and grep for all platform values in the codebase. Azure DevOps and Bitbucket Server are the most commonly omitted. _Caught by Copilot: PR #289 — standalone git host question omitted Azure DevOps; `CLANCY_GIT_PLATFORM` comment omitted `bitbucket-server`._
