...
The implementer supports fully autonomous operation via `/clancy:autopilot`. In this mode, Clancy:
...
```
.clancy/clancy-autopilot.js
  └─ while i < MAX_ITERATIONS:
       node .clancy/clancy-implement.js
...
The verification gate uses a specialist agent prompt (`packages/terminal/src/agents/verification-gate.md`) that understands how to interpret lint errors, test failures, and type errors, and apply targeted fixes without regressing other changes.
...
1. When `/clancy:implement` starts, it writes a lock file containing the ticket key, branch name, timestamp, and PID
