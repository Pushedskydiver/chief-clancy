...
- **`@chief-clancy/dev`** ships `/clancy:dev` and `/clancy:board-setup`. Dev needs board credentials to fetch tickets for autonomous execution via `/clancy:dev`. Board-setup is adapted from brief (same credential collection, same `.clancy/.env` output) with dev-specific text. Local-mode execution (without board credentials) runs through dev's pipeline — dev exposes the `--from <plan>` / `fromPath` + `localTicketSeed` infrastructure — but the user-facing surface lives in `@chief-clancy/terminal` as `/clancy:implement --from <plan>`, not as a dev-owned slash command.
