...
- **Function verb vocabulary.** Prefer the shortest, most-semantically-specific verb with established computing vocabulary. If no verb pulls its weight, drop the prefix and name the function after the returned value (noun-phrase form).
  ...
  - **Migration is deferred.** Existing callsites using dropped verbs — notably `compute*` and `attempt*` — rename in a post-Phase-5 workstream; new code follows the vocabulary immediately.
...
- **Type suffix conventions (project dialect).** For types that earn a name per the **Inline trivial types** rule: `*Opts` for options bags, `*Result` for discriminated-union returns, `*Ctx`/`*Context` for runtime context, `*Deps` for dependency-injection shapes, `*Fn` for function type aliases. No `I` prefix on interfaces, no `T` prefix on generics.
