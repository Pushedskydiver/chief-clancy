...
- **Function verb vocabulary.** Prefer the shortest, most-semantically-specific verb with established computing vocabulary. If no verb pulls its weight, drop the prefix and name the function after the returned value (noun-phrase form).
  - **Standard verbs:** `make*` (factory returning a closure), `create*` (single-call factory returning a fully-initialised object), `build*` (multi-step structured-value construction), `resolve*` (derivation with genuine fallback/lookup chain), `parse*` (unstructured → typed), `detect*` (environmental presence check, returns `T | undefined`), `ensure*` (idempotent side-effect), `fetch*` (external I/O), `find*` (search a pre-loaded collection), `is*/has*/can*` (boolean predicate), `wire*` (DI composition — `dep-factory/` dialect only).
  - **Dropped verbs:** `compute*/calculate*` (leaks mechanism — use noun-phrase form), `attempt*/try*` (redundant with discriminated-union result types — use the appropriate verb instead), generic `get*/set*` (prefer specific verbs or noun-phrase form; domain-specific accessors like `getPageTitle` are fine).
  - **Migration is deferred.** Existing `compute*` and `attempt*` callsites rename in a post-Phase-5 workstream; new code follows the vocabulary immediately.
...
The codebase today uses `{ ok: false, error: string }` in many sites — `packages/core/src/types/remote.ts` and the looser `PingResult` (`packages/core/src/types/board.ts:52`, `{ ok: boolean; error?: string }`) consumed by every board adapter's ping. Pipeline phases in `packages/dev/src/pipeline/` vary today (`error`, `reason`, or bare `ok` on failure). One outlier `{ success: false, error: { message } }` lives in `packages/core/src/board/detect-board.ts:33`. **New code uses the tagged-union shape above; existing sites migrate opportunistically when touched.** No forced-march refactor.
