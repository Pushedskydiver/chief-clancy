...
- **Extract a variable only when the name teaches the reader something.** Before hoisting to a named `const`, ask: "what does this name teach that the expression doesn't?" If the answer is `result`, `data`, `foo`, or a tautological restatement, inline it. For type declarations, the **Inline trivial types** rule takes precedence — a weak name is still an anchor for 2+-property shapes.
