...
- [ ] Function names follow the verb vocabulary: standard verbs per their semantic category; no `compute*/calculate*` (noun-phrase instead), no `attempt*/try*` (appropriate verb + discriminated-union result type), no generic `get*/set*` (domain-specific accessors are fine); new code only — existing migration is deferred
