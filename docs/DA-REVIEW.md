...
**When a rule body cites a compressed summary — a PROGRESS.md handoff entry, a `docs/history/SESSIONS.md` archive row, or a meta-finding carry-forward — verify against the primary source before landing.** Summaries elide context: a quantifier ("11 tests") may be a miscount at authorship; a severity tally ("3 MATERIAL") may drop a `+ 1 LOW` from the audit-trail; a `file:line` anchor may point at a bullet that has moved, or at a semantically-distinct line that downstream readers conflate with the primary catch. Primary sources by class: filesystem grep for counts, PR audit-trail comment for surrogate severity tallies, `git show <merge-sha>:<file>` for line-number claims. Cross-doc-citation discipline (prefer §section-name over line number) lives at [SELF-REVIEW.md §Consistency](SELF-REVIEW.md#consistency); this rule covers the orthogonal summary-vs-primary-source axis. Scope: rule bodies in policy docs + evidence clauses in spec drafts that feed them; ephemeral surfaces (commit messages, PR descriptions, handoff entries, archive rows) are exempt — they are the summaries the rule warns against citing. Pattern observed: [#376](https://github.com/Pushedskydiver/chief-clancy/pull/376) R1 B2 — a Session 112 PROGRESS.md miscount "11 integration tests" (actual 10 via `find packages/terminal/test/integration -name '*.test.ts'`) propagated via meta-finding carry-overs into a PR #376 rule-body draft; [#377](https://github.com/Pushedskydiver/chief-clancy/pull/377) surrogate F1 — a Session 113 PROGRESS.md "3 MATERIAL" summary of PR #373 (F4 LOW elided; audit-trail header reads `3 MATERIAL + 1 LOW`) inherited into the pre-merge `§Post-PR flow` rule body's aggregate ("17 MATERIAL + 4 LOW" → folded to `+ 5 LOW` via C2); [#378](https://github.com/Pushedskydiver/chief-clancy/pull/378) R\_{n+1} — Session 112 PROGRESS.md correctly authored both `CONVENTIONS.md:208` (primary catch, Integration-tests bullet) and `CONVENTIONS.md:210` (cross-doc-caller-sweep secondary anchor, Tracer-bullet-TDD — a TESTING.md caller) in the same entry; Session 113 narrative re-citation first conflated `:210` with the catch, and the conflation propagated through four subsequent PROGRESS.md handoffs (Sessions 114-117) into a PR #378 spec-draft rule body inherited directly from Session 115's carry-over per the Session 118 R\_{n+1} meta-finding; verified referent at PR #372 merge SHA `937903c` is L208 (Integration-tests bullet).
...
**Spec-grill folds count as rewrites** when they change a definition, taxonomy, classification, or rubric — the named target sections may not enumerate every place the term appears, and adjacent dependent prose can carry the old definition while the named targets carry the new one. See [REVIEW-PATTERNS.md §Spec grilling — Fold-incompleteness in multi-section folds](REVIEW-PATTERNS.md#fold-incompleteness-in-multi-section-folds).

### Cross-doc consistency sweep

Two discipline modes. **Reactive (sub-classes a–c)** — when the current round's scope spans more than one file via any of (a) downstream-cascade edits (CONVENTIONS → SELF-REVIEW / DA-REVIEW / `.github/copilot-instructions.md`), (b) cited-doc caller-claims (the spec or PR body asserts another doc's content by citation), or (c) co-edited files (a multi-file atomic diff) — read every file in scope at HEAD and verify cross-file consistency. **Proactive (sub-class d)** — when editing a named-edge concept at its source-of-truth, sweep the whole repo for all mentions regardless of diff scope; siblings outside the diff are, by construction, not reached by reactive triggers. Distinct from the intra-file §Post-restructure consistency sweep above; this covers every cross-file case. Reactive mode applies at every round that sweeps beyond the primary file — spec-grill R1 retrieval, R_n verification, per-commit DA on a multi-file commit, final-verification DA on a multi-file PR. Proactive mode applies author-side, pre-push. Companion: [SELF-REVIEW §Consistency](SELF-REVIEW.md#consistency) covers the apply-rule-to-own-draft companion discipline.
...
- **Sibling-doc kept-prose invalidation (proactive)** — a source-of-truth edit silently invalidates unmodified prose in sibling docs that described the prior state. Distinct from the three cases above: no cascade (siblings aren't edited in the diff), no caller-claim citation, no co-edit (siblings aren't in scope at all). Diff-scoped reviewers cannot catch this class by construction — the stale siblings are, by definition, not in the diff they review. When editing a named-edge claim (dependency chain, canonical list, policy rule, taxonomy enumeration), proactively grep-sweep the whole repo for all mentions of the edited concept — including synonym variants + arrow-form + phrase-form — and fold sibling updates into the same PR. No reactive catcher is codified — the surrogate's scope is intra-file-at-HEAD within files already in the diff (see [`copilot-surrogate.md`](../.claude/agents/copilot-surrogate.md): "Read each touched file at HEAD in full — not the diff"), so siblings outside the diff fall through. _Evidence: n=4 cumulative across Sessions 125-127 — PR #387 `eslint.config.ts` cleanup stranded pre-fix `docs/CONVENTIONS.md` kept prose (DA pre-merge caught, in-diff); PR #388 post-merge surrogate surfaced 4-doc sibling dependency-chain drift (surrogate + Claude post-surrogate `← wrapper` synonym grep together surfaced 5 stranded files — fix landed in PR #389); PR #390 surrogate + DA convergently surfaced `docs/GLOSSARY.md:10,19` kept prose after `.github/copilot-instructions.md` source-edit (fix in PR #391); PR #393 DA pass-0 surfaced `docs/decisions/PACKAGE-EVOLUTION.md` parenthetical-precision drift across the `← scan` / `← core, scan` / `← core, dev` arrow cluster (L35-40) that the author's phrase-form grep (searching prose enumerations of `brief, plan, and dev`) missed because the arrow-form was a different grep shape._
...
The `da-review` subagent's output is consumed in-chat by Claude for triage + folds. It must **not** post comments on the PR. The PR audit-trail slot is owned by the surrogate-findings-as-audit-comment flow per [DEVELOPMENT.md §Post-PR flow](DEVELOPMENT.md#post-pr-flow) `Copilot-surrogate dispatch` — both subagents return findings in-band; Claude posts the surrogate's findings as a PR comment (per `copilot-surrogate.md:17` "Claude owns posting"), while DA findings stay in-chat. Conflating the two clutters the PR timeline and muddles the audit-trail signal. Symmetric discipline: both subagents return in-band; only the surrogate's output becomes a PR comment (posted by Claude). When dispatching `@agent-da-review`, instruct the agent to return findings as the tool result; do not ask it to post a PR comment. If a DA comment ends up on a PR by accident, delete it — it is noise, not the audit trail. _Evidence: Session 128 novel n=1 — DA agent posted two review comments on [PR #395](https://github.com/Pushedskydiver/chief-clancy/pull/395) (pass-0 + pass-1 final-verification); Alex surfaced the skip, comments deleted, rule codified in this PR._
...
- [ ] No boundary violations (core importing from terminal, brief, plan, dev, scan, design, or chief-clancy)
