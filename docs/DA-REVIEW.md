...
- [ ] Annotations and `satisfies` preferred over `as`; `as` has a justification comment; no `as unknown as X` in production (tests: prefer `makeX()` builders); `!` only when locally provable
