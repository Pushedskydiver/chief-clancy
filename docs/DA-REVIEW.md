...
- [ ] Function names follow the verb vocabulary: standard verbs per their semantic category; no `compute*/calculate*` (noun-phrase instead), no `attempt*/try*` (appropriate verb + `Result<T, E>`), no generic `get*/set*`; new code only — existing migration is deferred
