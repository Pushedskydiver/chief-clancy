...
- [ ] No bare function references in array callbacks (`.map(fn)` → `.map((x) => fn(x))`) — enforced by `unicorn/no-array-callback-reference`; type-guard predicates and built-in constructors (`Boolean`, `Number`) exempt
