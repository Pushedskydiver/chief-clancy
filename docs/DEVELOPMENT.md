...
1. **Layering** (dependency direction): does this introduce a forbidden import? Drag heavy deps into a light-deps package? Cross the documented `core ← dev ← terminal ← chief-clancy` chain?
...
**Loading-instructions format:** when a session triggers handoff, the entry ends with a `### Session N+1 loading instructions` block enumerating (i) the order in which to read files, (ii) the authoritative source-of-truth for the active workstream, (iii) where execution should resume. The most recent `### Session N+1 loading instructions` block in PROGRESS.md (top of detail band) is the canonical template — currently the lean shape per the No-dogfeeding-in-LI rule below.

**No-dogfeeding-in-LI rule:** loading-instructions blocks must not restate rules already codified in `docs/*.md`. Pointer-only — `CLAUDE.md §Key docs` Before-X-read-Y pattern + `docs/INDEX.md` route to the relevant doc. The block carries session-specific direction (workstream pointer, decision branches, carry-overs); codified rules live in their canonical homes and Claude reads them when needed. API surface alerts (TypeScript types or runtime invariants Claude won't see without reading the file) are legitimate session-resident content and stay. Origin: Session 148 save → next-session lean LI applied at handoff (Sessions 149-150 onward).
...
**Session-start firing is the primary trigger; preemptive-at-handoff is the secondary mode.** Check `PROGRESS.md`'s detailed-sessions band (everything between `## Next workstreams` and `## Session archive`) at session load, **before proposing any primary workstream**. If it contains more than 5 entries OR the band exceeds ~10k tokens, compress the oldest entry — or oldest entries (multi-session catch-up is one §12 trigger, not several) — to one-line rows in [`history/SESSIONS.md`](history/SESSIONS.md) before continuing with workstream selection.
...
**Why session-start primary.** Preemptive-at-handoff is helpful when it fires — it folds into the same direct-to-main `PROGRESS.md` commit that ships the Session-N handoff entry, and the next session opens with N ≤ 5 detailed entries and zero first-turn maintenance tax. But it is empirically unreliable: Sessions 104-112 sustained n=10 consecutive preemptive applications, after which applications became intermittent. Session 152 close consciously deferred archival to Session 153 (per the Session 152 handoff commit body `57d2946` — "Session 148 §12 archive deferred to Session 153"), but the deferral did not fire at Session 153 close, nor at any of the next four session closes — accumulating a 6-session backlog (Sessions 148-153 unarchived), only caught when Alex asked at Session 158 load (commit `70d399a` documents the catch-up). Deferred-at-close carry-overs depend on subsequent sessions firing them; heavy sessions are the same sessions where archival matters most and where carry-overs are most likely to be skipped again. The at-load check is non-optional and fires before any workstream selection; preemptive-at-handoff is encouraged but cannot substitute for it.

**Embedded loading-instructions blocks.** A session entry's `### Session N+1 loading instructions` block is tied to its parent's lifecycle. When the parent collapses to a one-line `history/SESSIONS.md` row, the LI block must be deleted from PROGRESS.md as part of the same archival commit (the session it directed has run; the LI is historical and recoverable via `git log -p PROGRESS.md`). **Retroactive cleanup**: if a pre-rule LI in the active detail band restates `docs/*.md`-codified rules per §Loading-instructions format, the dogfeeding bullets may be trimmed in-place even before parent archival — the rest of the LI body (read-instructions, sha-verify, push-hygiene, Dependabot-check, primary-workstream, decision-branches, readback, carry-overs, alex-redirect) stays intact. **For LIs whose directed session has already run** (functionally historical), API surface alerts may also be trimmed since the parent session's main-body retains the API surface info; for LIs still directing a future session, the §Loading-instructions format carve-out applies and API surface alerts stay. Deletion of the entire LI block remains tied to parent archival.
...
**Post-fold audit pass.** After landing a fold that changes a definition, taxonomy, classification, or rubric, run the [§Post-restructure consistency sweep](DA-REVIEW.md#post-restructure-consistency-sweep) on the spec body — grep the whole spec for the changed term and re-read every hit. Adjacent dependent prose may share the term without being enumerated as a fold target. See [REVIEW-PATTERNS.md §Spec grilling — Fold-incompleteness in multi-section folds](REVIEW-PATTERNS.md#fold-incompleteness-in-multi-section-folds) for evidence.
...
**Bounded-fold 2-round arc.** When a fold doesn't change a definition, taxonomy, classification, or rubric that propagates across multiple spec sections — i.e., the [3-axis fold-incompleteness sweep](REVIEW-PATTERNS.md#fold-incompleteness-in-multi-section-folds) returns clean — the grill arc converges in 2 rounds: R1 (discovery) → fold → R_n (verification). Proceed directly from R1 fold to R_n verification on bounded folds; padding with an R2 discovery round is unwarranted absent a concrete unresolved finding from R1. The 2-round arc does NOT apply to ramifying folds — folds that change a definition, taxonomy, classification, or rubric that propagates — which follow the **Post-fold audit pass** above + iterate discovery until ripple is contained per the same fold-incompleteness rule. This rule refines the canonical iteration-cost baseline below at [§Plan-stage mandatory HITL](#plan-stage-mandatory-hitl) for the bounded-fold subclass at the lower bound. Evidence — n=5 across 3 sessions (1 author, 1 cohort): Step D Layer 3 follow-up analysis (Session 140, gitignored research-spec); RATIONALIZATIONS procedural-reinforcement entry ([PR #407](https://github.com/Pushedskydiver/chief-clancy/pull/407), Session 141); empirical-test-plan v7.1 amendment (Session 141, gitignored research-spec); REVIEW-PATTERNS fold-incompleteness 3-axis generalization ([PR #408](https://github.com/Pushedskydiver/chief-clancy/pull/408), Session 141); dependency-automation γ-spec ([PR #409](https://github.com/Pushedskydiver/chief-clancy/pull/409) + [PR #410](https://github.com/Pushedskydiver/chief-clancy/pull/410), Session 143, gitignored research-spec). All 5 produced 0 BLOCKING + 0 MATERIAL at R_n; 3 of 5 closed sub-LOW-floor items via post-LOCK polish, 1 of 5 (Step D) deferred a single sub-floor item to a downstream promotion PR. None required an R2 discovery round.
...
1. **Copilot review** — request explicitly after push; the repo does not auto-trigger Copilot on PR open. Use `gh api -X POST repos/{owner}/{repo}/pulls/{n}/requested_reviewers -f 'reviewers[]=copilot-pull-request-reviewer[bot]'` (the app login resolves to `Copilot` in `requested_reviewers`). Claude then waits for the review to complete or for 10min (dial; adjust based on post-merge observations) after CI green, whichever first.
   ...
   **Copilot-unreachable detection.** The assignment can silently drop on monthly-quota / rate-limit / transient failures — Copilot doesn't post a status comment (monthly-quota observed on PR #368 R3; rate-limit and transient expected to behave the same). Two observable signals from Claude's side (observed behaviour, not documented API contract — revisit if either stops firing): (a) the `gh api -X POST .../requested_reviewers` response body's `requested_reviewers` field returns `[]` (empty); (b) no `copilot_work_started` event appears in the PR timeline within ~4 min of the request (dial; adjust based on post-merge observations). If both fire, classify as unreachable — don't retry the assignment. Classification is informational; the existing [§Auto-merge criteria](#auto-merge-criteria) Copilot gate branches still own merge permission (prior-round-on-record satisfies the "posted a review + threads resolved" branch; no-prior-round waits out the 10min hang fallback). Exercised live on [PR #368](https://github.com/Pushedskydiver/chief-clancy/pull/368) R3 (2026-04-19, monthly quota hit after R1+R2 posted).

   **Copilot-surrogate dispatch.** The [`copilot-surrogate`](../.claude/agents/copilot-surrogate.md) agent dispatches when either of the following triggers fires (if both apply, the surrogate still runs once):
   - **Drift-fix PR (mandatory) — any commit in the PR uses type `fix(docs)` or `fix(decisions)`.** Dispatch the surrogate regardless of Copilot classification. Match against commit titles only; revert commits (`Revert "..."`) don't count. Evidence: 5 consecutive drift-fix PRs ([#372](https://github.com/Pushedskydiver/chief-clancy/pull/372), [#373](https://github.com/Pushedskydiver/chief-clancy/pull/373), [#374](https://github.com/Pushedskydiver/chief-clancy/pull/374), [#375](https://github.com/Pushedskydiver/chief-clancy/pull/375), [#376](https://github.com/Pushedskydiver/chief-clancy/pull/376)) surfaced 17 MATERIAL + 5 LOW factual-claim findings the diff-scoped DA stack missed. Dominant class is kept-prose drift — unmodified paragraphs written under a prior tree state — which diff-scoped reviewers systematically miss.
   - **Copilot-unreachable fallback — Copilot classified UNREACHABLE per the `Copilot-unreachable detection` paragraph above.** Dispatch the surrogate as a substitute for Copilot's structural reads-HEAD-not-diff scope.

   In both cases the surrogate reads each file in the PR diff at HEAD (not diff-scoped) and returns factual-claim findings in-band; Claude then posts a PR comment summarising them (when both triggers fire, cite the mandatory trigger in the audit comment; include an explicit zero-findings note when applicable — the comment is the audit trail) and applies the standard Findings triage (step 2 below) + Resolve-after-reply (step 3) flow. **Auto-merge gate is unaffected.** Surrogate findings are informational — the [§Auto-merge criteria](#auto-merge-criteria) Copilot gate still requires Copilot's own review or the 10min hang fallback per the `Copilot-unreachable detection` paragraph above, independent of surrogate output. The surrogate closes the kept-prose factual-drift class the diff-scoped DA stack misses — validated against [PR #370](https://github.com/Pushedskydiver/chief-clancy/pull/370) pre-fix state (Session 110 pilot, 2026-04-20): 11 grep-verified factual-drift findings, reproducing all four Seed-109 findings from the post-merge `fix(decisions)` commits (`88eba3d` + `e57fcde`) plus seven additional findings Session 109's post-merge audit missed (five landed as `23d6e86`; two clarified as target-state not drift).

   **Dispatch timing + audit-trail discipline.** The surrogate is a **PR-side review step, not a Claude-side verification check**. It runs during step 1 above (the review gate: after push, before merge) — it has a protocol-mandated slot, not a flexible "whenever Claude wants to verify" window. Two complementary disciplines: **(a) Pre-merge timing** — dispatch the surrogate while the PR is open, after push, as part of step 1; dispatching post-merge defeats the gate. **(b) Audit-comment channel** — always post the findings as a PR comment before merge, even when consuming them in-chat for triage; the PR comment IS the audit trail per the zero-findings-included clause above, not the in-chat consumption. Skipping either (post-merge dispatch, or in-chat-only consumption without the PR comment) is a **surrogate-protocol-failure** — restore retroactively (post the audit comment even if the merge already happened) and surface the skip to the user. _Evidence: Session 126 PR [#388](https://github.com/Pushedskydiver/chief-clancy/pull/388) novel n=1 — both disciplines skipped on first drift-fix PR (dispatched post-merge; consumed in-chat without posting audit comment); Alex surfaced the audit-comment gap first, then the timing gap on the follow-up PR; both restored retroactively. Session 127 held discipline across 7 dispatches with 0 recurrences._
...
- **Per-commit DA + final verification DA both returned READY TO MERGE.** Two DA passes per [§P3 Review discipline](#review-discipline); iterating beyond final verification has diminishing returns.
  - **Dependabot DA-waiver.** PRs authored by `dependabot[bot]` AND carrying the `dependabot-autoskip` label satisfy this gate without DA dispatch — machine-generated lockfile bumps have no design surface for DA to grill, and a separate dep-triage workflow (shipping as γ-PR-2) will surface failures. Other gates (CI green, stability window, Copilot, no merge conflicts, type+package labels) apply per their respective bullets; exceptions apply per the §Exceptions list below (which carries Dependabot-specific carve-outs for blast-radius + Lockfile-hand-edit + a new major-bump exception). **Self-review is dropped for bot-authored PRs** — the gate's purpose is to catch the author's own line-level slips, which doesn't apply when there's no Claude author; Copilot's PR-open review remains the Copilot gate, unchanged. Waiver scope is narrow: BOTH the GitHub-verified bot author AND the label must hold — a renamed bot can't impersonate, and a non-Dependabot PR carrying the label still fails the author check. This carve-out suppresses DA dispatch only; auto-merge enablement is governed separately by the §Blast-radius Dependabot package-manifest carve-out + the §Dependabot major bump exception (both below). Patch+minor bot PRs flow through to auto-merge; major bot PRs route to Alex-handoff.
...
- **Semver**:
  ...
  - `pnpm changeset status --since=origin/main` exits non-zero (no new changeset since `origin/main`) AND the PR lacks a `skip-changeset` label. Canonical check per [changesets/changesets docs](https://github.com/changesets/changesets/blob/main/docs/checking-for-changesets.md); the docs use `--since=main` as shorthand — we use `origin/main` explicitly since that's the base CI resolves in practice. `skip-changeset` is the human-in-the-loop escape hatch for test-only / docs-only / infra-only PRs that legitimately need no changeset.
    ...
    **Private-package carve-out.** A non-zero exit caused **solely** by changed packages that are all `private: true` does not trip this exception. Private packages are never published — `changesets/action` skips them (see [§Release Flow "New packages start private"](#release-flow)) — so they legitimately need no changeset, yet changesets' default `privatePackages.version: true` still reports them as "changed but no changesets found". Confirm **every** `packages/*` directory touched in the diff is `private: true` (today only `@chief-clancy/design` qualifies — verify against `package.json`, not from memory); if **any** public package changed without a changeset, the exception fires as normal. No `skip-changeset` label is required in the private-only case — the label stays the escape hatch for docs-only/infra-only/test-only PRs that touch a _public_ package's tree. _Evidence: the Phase F rework's design-only slices [#492](https://github.com/Pushedskydiver/chief-clancy/pull/492) / [#494](https://github.com/Pushedskydiver/chief-clancy/pull/494) / [#495](https://github.com/Pushedskydiver/chief-clancy/pull/495) all Claude-merged through this false-positive._
...
- **Dependabot major bump** — fires when PR author is GitHub-verified `dependabot[bot]` AND the PR carries the `dependabot-semver-major` label (added by [`dep-classify.yml`](../.github/workflows/dep-classify.yml) per `dependabot/fetch-metadata` `update-type` highest-semver-wins for grouped PRs). Major-version dependency upgrades typically include API breakage that requires human-judged migration; auto-merge is unsafe even when CI is green. Pre-1.0 (`0.x → 0.y` where y > current) and pre-release transitions (e.g. `5.0.0-beta.1 → 5.0.0`) follow `fetch-metadata`'s classification — the workflow output is canonical. If the classification step did not run or failed, the absence of all three `dependabot-semver-*` labels causes the §Blast-radius Dependabot package-manifest carve-out above to fail closed, routing the PR to Alex regardless.

  **Work split during Alex-handoff.** Claude prepares the PR before merge — worktree-probes the branch (`git worktree add` + `pnpm install` + the full pre-push quality suite to surface all errors at once instead of iterating CI failures), reads each major's release notes for breaking changes + required migrations, and opens any cleanup-precursor PRs needed (lint, type, behavior migrations) via the normal auto-merge path. Migrations do NOT bundle into the Dependabot branch — Dependabot stops rebasing once non-bot commits land on its branch (freezing the PR at a stale tip), so prep work must ship as separate PRs against main; Dependabot then rebases its branch over the new main cleanly. Claude surfaces the complete picture — breaking-change summary, precursor-PR list, recommended merge order — and Alex reviews + presses merge once both parties are confident. Precedent: Session 153 PR [#429](https://github.com/Pushedskydiver/chief-clancy/pull/429) (TS 5.9.3 → 6.0.3 major) needed precursor PRs [#430](https://github.com/Pushedskydiver/chief-clancy/pull/430) + [#431](https://github.com/Pushedskydiver/chief-clancy/pull/431) to clear 36 `@typescript-eslint/no-unnecessary-type-assertion` errors before #429 could merge cleanly.
