...
1. **Copilot review** — request explicitly after push; the repo does not auto-trigger Copilot on PR open. Use `gh api -X POST repos/{owner}/{repo}/pulls/{n}/requested_reviewers -f 'reviewers[]=copilot-pull-request-reviewer[bot]'` (the app login resolves to `Copilot` in `requested_reviewers`). Claude then waits for the review to complete or for 10min (dial; adjust based on post-merge observations) after CI green, whichever first.
   ...
   **Copilot-surrogate fall-through.** When classified UNREACHABLE, dispatch the [`copilot-surrogate`](../.claude/agents/copilot-surrogate.md) agent as a fall-through review pass. The surrogate reads each file in the PR diff at HEAD (not diff-scoped) and returns factual-claim findings in-band; Claude then posts a PR comment summarising them (including an explicit zero-findings note when applicable — the comment is the audit trail) and applies the standard Findings triage (step 2 below) + Resolve-after-reply (step 3) flow. **Auto-merge gate is unchanged** — surrogate findings are informational; the [§Auto-merge criteria](#auto-merge-criteria) Copilot gate still requires Copilot's own review or the 10min hang fallback per the unreachable-detection paragraph above. The surrogate closes the kept-prose factual-drift class the diff-scoped DA stack misses — validated against [PR #370](https://github.com/Pushedskydiver/chief-clancy/pull/370) pre-fix state (Session 110 pilot, 2026-04-20): 11 grep-verified factual-drift findings, reproducing all four Seed-109 findings from the post-merge `fix(decisions)` commits (`88eba3d` + `e57fcde`) plus seven additional findings Session 109's post-merge audit missed (five landed as `23d6e86`; two clarified as target-state not drift).
