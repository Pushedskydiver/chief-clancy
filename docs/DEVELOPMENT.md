...
**Why session-start primary.** Preemptive-at-handoff is helpful when it fires — it folds into the same direct-to-main `PROGRESS.md` commit that ships the Session-N handoff entry, and the next session opens with N ≤ 5 detailed entries and zero first-turn maintenance tax. But it is empirically skippable under heavy session load: Sessions 104-112 sustained n=10 consecutive preemptive applications, then the streak broke at Session 152 close (5 PRs + workstream retirement + reframe — protocol step silently dropped) and stayed broken for 6 consecutive sessions, accumulating a 6-session backlog (148-153) only caught when Alex asked at Session 158 load (commit `70d399a` documents the catch-up). Heavy sessions are the same sessions where archival matters most, so a "primary at handoff, reactive at load" framing leaves the load-bearing case unprotected. The at-load check is non-optional and fires before any workstream selection; preemptive-at-handoff is encouraged but cannot substitute for it.
