...
- **Per-commit DA + final verification DA both returned READY TO MERGE.** Two DA passes per [§P3 Review discipline](#review-discipline); iterating beyond final verification has diminishing returns.
  - **Dependabot DA-waiver.** PRs authored by `dependabot[bot]` AND carrying the `dependabot-autoskip` label satisfy this gate without DA dispatch — machine-generated lockfile bumps have no design surface for DA to grill, and a separate dep-triage workflow (shipping as γ-PR-2) will surface failures. Other gates (CI green, stability window, **Self-review**, Copilot, no merge conflicts, type+package labels) and exceptions (size, semver, blast-radius, **`Lockfile hand-edit`**, Release PR, Revert, HITL signal) all still apply unchanged — for bot-authored PRs the merger's review serves as the Self-review gate. Waiver scope is narrow: BOTH the GitHub-verified bot author AND the label must hold — a renamed bot can't impersonate, and a non-Dependabot PR carrying the label still fails the author check. Note: this carve-out only suppresses DA dispatch; it does NOT enable Dependabot-PR auto-merge — typical Dependabot pure-transitive PRs trip the `Lockfile hand-edit` exception below and route to Alex-handoff regardless. Suppressing that exception for `dependabot[bot]` is δ-scope work, not γ.
