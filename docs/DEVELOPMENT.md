...
- **Dependabot major bump** — fires when PR author is GitHub-verified `dependabot[bot]` AND the PR carries the `dependabot-semver-major` label (added by [`dep-classify.yml`](../.github/workflows/dep-classify.yml) per `dependabot/fetch-metadata` `update-type` highest-semver-wins for grouped PRs). Major-version dependency upgrades typically include API breakage that requires human-judged migration; auto-merge is unsafe even when CI is green. Pre-1.0 (`0.x → 0.y` where y > current) and pre-release transitions (e.g. `5.0.0-beta.1 → 5.0.0`) follow `fetch-metadata`'s classification — the workflow output is canonical. If the classification step did not run or failed, the absence of all three `dependabot-semver-*` labels causes the §Blast-radius Dependabot package-manifest carve-out above to fail closed, routing the PR to Alex regardless.
  ...
  **Work split during Alex-handoff.** Claude prepares the PR before merge — worktree-probes the rebased branch (`git worktree add` + `pnpm install` + the full pre-push quality suite to surface all errors at once instead of iterating CI failures), scans each major's release notes for breaking changes + required migrations, and opens any cleanup-precursor PRs needed (lint, type, behavior migrations) via the normal auto-merge path. Migrations do NOT bundle into the Dependabot branch — Dependabot rewrites the branch on rebase. Claude surfaces the complete picture — breaking-change summary, precursor-PR list, recommended merge order — and Alex reviews + presses merge once both parties are confident. Precedent: Session 153 PR [#429](https://github.com/Pushedskydiver/chief-clancy/pull/429) (TS 5.9.3 → 6.0.3 major) needed precursor PRs [#430](https://github.com/Pushedskydiver/chief-clancy/pull/430) + [#431](https://github.com/Pushedskydiver/chief-clancy/pull/431) to clear 36 stale-bundled-deps lint errors before #429 could merge cleanly.
