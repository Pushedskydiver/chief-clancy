...
Applies to every grill — both P1 spec grills and P3 plan grills. The grill loop is not one kind of thing; it's two:
...
Pilot evidence (Barrier [PR #305](https://github.com/Pushedskydiver/chief-clancy/pull/305), PR5 [#306](https://github.com/Pushedskydiver/chief-clancy/pull/306), PR6 [#308](https://github.com/Pushedskydiver/chief-clancy/pull/308)): the verification pass consistently catches defects the discovery loop converged past — self-referential cross-section inconsistencies, stale line-number references, logical contradictions that individual-claim checks missed. Three independent runs validate the pattern; one-pass grilling does not.
...
- **During execution: per-commit DA + final verification DA.** Two passes, distinct prompts — per-commit DA runs against each commit before the next commit lands (discovery brief, catches mechanical errors within one blast radius); final verification DA runs against the PR's full diff before push (evaluative-skeptical, confirms merge-readiness).
- **Two DA passes, not N.** Iterating DA beyond final verification has diminishing returns against DA's own error model: the same blind spots recur across re-runs. The designed review stages are plan grill → per-commit DA → final verification DA → Copilot. Don't patch a DA-miss by adding a fourth DA pass; Copilot's post-push pass is the third reviewer by design, and a DA-miss Copilot catches is part of the stack working, not a signal to re-run DA.
- **Verification DA prompt includes an explicit cross-section-consistency check** when the PR revises or adds a definitional rule (classification table, named category, or prose using absolute quantifiers like "every", "no", "always", "never"). Evidence: on PR5 + PR6, most of the 13 Copilot findings across the two PRs (at least 7 strictly; up to 9 under a looser rubric) were absolute-phrase-contradicts-another-section-of-same-diff — the dominant failure class for structural-doc PRs. Fold the check into the verification prompt, not a separate review stage.
...
- **Further execution-mechanics rules pending a second mechanical-refactor run** — [Barrier-Core](CONVENTIONS.md#migration-state--core) is the next candidate. Barrier established the pattern; promotion into this section happens once a second run reproduces the two-phase catch pattern (verification round surfaces ≥1 defect discovery missed). Until then, Barrier-specific patterns (madge cycle-baseline diff, name-collision audit for bulk sed, "restart from manifest on late decision invalidation") stay in the Barrier pilot notes and in SELF/DA where already captured.
