...
- `/clancy:implement --from <plan.md>`: `runLocalMode` (`packages/dev/src/entrypoints/dev.ts:168`) synthesises a ticket from the plan file via `localTicketSeed` and runs the pipeline with a no-op board (no API calls). The workflow spec (`packages/terminal/src/roles/implementer/workflows/implement.md:54`) instructs Claude to append `--skip-feasibility` to the script arguments, but the runtime does not implement a `--skip-feasibility` flag — the spec-level skip is a workflow directive to Claude, not a runtime gate
- `/clancy:implement --from <dir> --afk` (batch): **runtime wiring deferred**. The workflow spec describes iterating plan files with marker filtering, but neither `listPlans` nor any directory-iteration logic is called from `packages/dev/src/entrypoints/loop.ts` or `dev.ts`. Today the flag is a convention handled by Claude Code (the workflow) rather than by the runtime
...
- **Marker verification — currently deferred**: the verifier `checkApprovalStatus()` exists in `packages/dev/src/lifecycle/plan-file/plan-file.ts:144` and reads the marker, re-computes SHA-256 of the plan file, and compares against the marker's `sha256=` value. However, it has **zero callers in the pipeline** — `runLocalMode` → `wirePreflight` → `runLocalPreflight` only checks `isGitRepo`, and neither single-plan nor batch modes reference the marker. Per `approve-plan.md:412-418`, this gate is **intentionally deferred** until a dedicated verifier ships. Today approval is enforced manually (the user or Claude Code reads the marker and refuses to apply on mismatch) or by workflow convention. Treat the marker as a **write-side contract only**
