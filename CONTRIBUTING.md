...
The monorepo has eight packages:
...
- **`@chief-clancy/design`** — standalone AI-design tool (variant generation, canvas, design-system documentation). No core/terminal deps. Phase F in-progress, not yet published.
...
**Dependency direction:** core ← dev ← terminal ← chief-clancy. Brief, plan, scan, and design are standalone. Enforced by ESLint. Core must never import from terminal.
